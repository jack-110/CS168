...
class TestDVRouterBase(unittest.TestCase):
    ...
    def _set_up(
        ...
        call_time_error = AssertionError(
            "DO NOT call time.time() for timestamps; " "call api.current_time() instead"
        )
        ...
        current_time_patch = patch(
            "sim.api.current_time", side_effect=lambda: self._current_time
        )
        ...
        assert not (
            DVRouter.SPLIT_HORIZON and DVRouter.POISON_REVERSE
        ), "Split horizon and poison reverse can't be on"
        ...
        self.assertEqual(
            self.router.POISON_ON_LINK_DOWN,
            poison_on_link_down,
            "POISON_ON_LINK_DOWN flag doesn't match",
        )
        self.assertEqual(
            self.router.SPLIT_HORIZON, split_horizon, "SPLIT_HORIZON flag doesn't match"
        )
        self.assertEqual(
            self.router.SEND_ON_LINK_UP,
            send_on_link_up,
        ...
        for port, packets in sent_packets.items():
            for packet in packets:
                ...
                if old is None:
                    ...
                    self.assertEqual(old.dst, dst, "BUG: route ad dst differ")
                    self.assertEqual(
                        old,
                        route,
    ...
    def _add_test_routes_proper(self):
        ...
        self._assert_table_equal(
            {
                h1: TableEntry(
                    dst=h1, port=1, latency=8, expire_time=94 + self.router.ROUTE_TTL
                ),
                h2: TableEntry(
                    dst=h2, port=3, latency=5, expire_time=100 + self.router.ROUTE_TTL
                ),
            }
        )
    ...
    def _compute_dict_diff(self, d, expected, singular, plural):
        ...
        del missing
        incorrect = {
            k for k in set(d.keys()) & set(expected.keys()) if d[k] != expected[k]
        }
    ...
    def _assert_table_equal(self, expected):
        ...
        t = self.router.table
        self.assertIsInstance(t, Table, "{} is not a Table".format(t))

        failure = self._compute_dict_diff(
            t, expected, singular="table entry", plural="table entries"
        )
    ...
    def _assert_route_ads_sent(self, sent, expected_sets):
        ...
        for routes in expected_sets.values():
            ...
            self.assertEqual(
                len(routes),
                len(set(r.dst for r in routes)),
                "BUG: duplicate destination in expected_sets",
            )

        expected = {
            port: {r.dst: r for r in routes} for port, routes in expected_sets.items()
        }
        ...
        for port in sorted(sent.keys()):
            ...
            if failure:
                failed = True
                message += (
                    "\n\nMISMATCH detected for route advertisements sent to port %d:"
                    % port
                )
...
class TestStarterCode(TestDVRouterBase):
    ...
    def test_init(self):
        ...
        self.assertDictEqual(
            ...
            {},
            "link_latency isn't empty: {}".format(
                self.router.ports.get_underlying_dict()
            ),
        )
        self._assert_table_equal({})
...
class TestHandleAdvertisement(TestDVRouterBase):
    ...
    def test_single_neighbor(self):
        ...
        self.router.handle_route_advertisement(h1, port=1, route_latency=2)
        self.router.handle_route_advertisement(h2, port=1, route_latency=0)
        self.router.handle_route_advertisement(h3, port=1, route_latency=0.5)
    ...
    def test_multiple_neighbors(self):
        ...
        self.router.handle_route_advertisement(h2, port=1, route_latency=2)
        self.router.handle_route_advertisement(h3, port=1, route_latency=3)

        self.router.handle_route_advertisement(h2, port=3, route_latency=5)
        self.router.handle_route_advertisement(h3, port=3, route_latency=7)
    ...
    def test_ties(self):
        ...
        self.router.handle_route_advertisement(h1, port=3, route_latency=4)
        self.router.handle_route_advertisement(h2, port=3, route_latency=5)
    ...
    def test_update_same_port_shorter(self):
        ...
        h1, h2, h3 = self.h1, self.h2, self.h3
        self.router.handle_route_advertisement(h2, port=3, route_latency=1)
    ...
    def test_update_same_port_longer(self):
        ...
        h1, h2, h3 = self.h1, self.h2, self.h3
        self.router.handle_route_advertisement(h2, port=3, route_latency=5.25)
    ...
    def test_update_alternate_port_longer_same_port(self):
        ...
        h1, h2, h3 = self.h1, self.h2, self.h3
        self.router.handle_route_advertisement(h2, port=3, route_latency=6)
...
class TestInfiniteLoops(TestDVRouterBase):
    ...
    def test_propagate_poison(self):
        ...
        with self._patch_send_routes(all_ports={1, 2, 3, 10}, allow_dup=True) as ads:
            self.router.handle_route_advertisement(
                route_dst=h2, port=3, route_latency=INFINITY
            )
            self.router.send_routes(force=True, single_port=None)
        self._assert_route_ads_sent(
            ads,
    ...
    def test_handle_poison_no_route(self):
        ...
        self.router.handle_route_advertisement(
            route_dst=h1, port=1, route_latency=INFINITY
        )
        self.router.handle_route_advertisement(
            route_dst=h1, port=3, route_latency=INFINITY
        )
...
class TestRoutePoisoning(TestDVRouterBase):
    ...
    def test_expire_routes_single(self):
        ...
        self._assert_route_ads_sent(
            ads,
            {
                ...
                3: {Route(h1, INFINITY), Route(h2, 11), Route(h3, 14), Route(h4, 2)},
                10: {Route(h1, INFINITY), Route(h2, 11), Route(h3, 14), Route(h4, 2)},
            },
        )
    ...
    def test_expire_routes_multiple_periodic_poison(self):
        ...
        self._assert_route_ads_sent(ads, {port: expected for port in ports})
...
class TestTriggeredIncrementalUpdates(TestDVRouterBase):
    ...
    def test_new_route(self):
        ...
        self._set_up_routes(
            sub_poison_reverse=False,
            sub_poison_expired=False,
            sub_poison_on_link_down=False,
            sub_split_horizon=True,
        ),
        h1 = self.h1
    ...
    def test_shortest_route_longer(self):
        ...
        h2 = self.h2
        self._test_shortest_route_longer(
            expected_advertisements={
                1: {Route(h2, 12)},
                2: {Route(h2, 12)},
    ...
    def test_non_shortest_route_shorter(self):
        ...
        h1 = self.h1
        self._test_non_shortest_route_shorter(
            expected_advertisements={
                1: {Route(h1, 4)},
                2: {Route(h1, 4)},
                3: set(),
    ...
    def test_consecutive(self):
        ...
        self._set_up(False, False, False, True, True, True)
        ...
        with self._patch_send_routes(all_ports={1, 2, 3}) as ads:
            self.router.handle_route_advertisement(h1, port=1, route_latency=3)
        self._assert_route_ads_sent(
            ads, {1: set(), 2: {Route(h1, 8)}, 3: {Route(h1, 8)}}
        )

        with self._patch_send_routes(all_ports={1, 2, 3}) as ads:
            self.router.handle_route_advertisement(h2, port=1, route_latency=6)
        self._assert_route_ads_sent(
            ads, {1: set(), 2: {Route(h2, 11)}, 3: {Route(h2, 11)}}
        )
        ...
        with self._patch_send_routes(all_ports={1, 2, 3}) as ads:
            self.router.handle_route_advertisement(h2, port=3, route_latency=2)
        self._assert_route_ads_sent(
            ads, {1: {Route(h2, 5)}, 2: {Route(h2, 5)}, 3: set()}
        )
    ...
    def test_handle_link_down_update(self):
        ...
        h1 = self.h1
        self._test_handle_link_down_update(
            expected_advertisements={
                1: set(),
                2: set(),
                3: set(),
    ...
    def test_handle_link_down_update_multiple(self):
        ...
        self._test_handle_link_down_update_multiple(
            expected_advertisements={
                2: set(),
                3: set(),
                10: set(),
...
def main():
    test_cases = (
        ...
        TestForwarding,
        TestAdvertise,
        TestHandleAdvertisement,
        TestRemoveRoutes,
        TestSplitHorizon,
        TestPoisonReverse,
        TestInfiniteLoops,
        TestRoutePoisoning,
        TestTriggeredIncrementalUpdates,
    )
    ...
    parser.add_argument(
        "step_num",
        type=int,
        choices=range(len(test_cases)),
        default=len(test_cases) - 1,
        nargs="?",
        help="run tests for the first x steps " "(runs all tests if omitted)",
    )
    parser.add_argument(
        "--verbose",
        "-v",
    ...
    for stage, test_case in enumerate(test_cases[: args.step_num + 1]):
        eprint("********** Stage {}: {} **********".format(stage, test_case.__name__))
    ...
    for stage in range(1, len(test_cases)):
        if stage > args.step_num:
            eprint(
                "\tStage {0} {1: <35}:  tests not run".format(
        ...
        eprint(
            "\tStage {0} {1: <35}: {2: >2} / {3: <2} passed{4}".format(
                stage, test_cases[stage].__name__, successes[stage], totals[stage], msg
            )
        )
